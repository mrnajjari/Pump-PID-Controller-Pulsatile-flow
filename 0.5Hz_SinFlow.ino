// This is the PID controller program for Arduino micro-controller that controls a pump to produce a periodic signal consist of 1 sin wave with 0.5 Hz frequencyas a reference signal. The pump is designed to accept 0-5 V signal for setting the velocity.
...
const int Is_Gain_Schedule = 0;     // if ==0 then NO gain scheduling, if ==1 then gain scheduling is activated.

// --- Reference signal (Desired signal) parameters:
const int Ref_Sig_T = 2000; 											// This is the period of the Reference signal in mili s
const int num_Step_Ahead = 38; 											// This is the number of points that the PID controller is looking ahead in reference signal to compensate the delay
const float Time_Step = 2.2; 											// This is the number of the elements in the Reference signal vector (This can not be grater than about 1600 because the memory is not enough)
...
void loop() {
  ...
  if (Is_Gain_Schedule == 1) {
    ...
    if (T1 >= 0 && T1 <= 235) {   //235
      ...
      kp = 28.0 / 10000;
      ki = ki - 0.137 / 1000000.0;

    }
    if (T1 > 740 && T1 <= 999) {
      ki = ki + 0.097 / 1000000.0;
...
//  Serial.print(T1 / 909.*2, 3); // Serial.print does not go to NEXT LINE but Serial.println goes to NEXT LINE
//  Serial.print("\t");
